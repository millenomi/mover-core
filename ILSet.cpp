...
void ILSet::removeAllObjects() {
	size_t count = _content.count();
	
	ILObject* allObjects[ count ];
	_content.getAllValues((void**) allObjects);
	
	size_t i; for (i = 0; i < count; i++) {
		_content.removeValue((void*) allObjects[i]);
	}
}
